...
## Project Structure
- **Root directory**: Jupyter notebooks for hands-on learning (numbered sequentially)
  - `00_setup.ipynb`: Download and extract the Oberhollenzer dataset (run once)
  - `01_eda_cpt.ipynb`: Data exploration and profiling with ydata-profiling
  - `02_metrics_and_ml_intro.ipynb`: ML fundamentals and evaluation metrics
  - `03_training_evaluation.ipynb`: Model training with sklearn, imblearn pipelines
...
- First run `00_setup.ipynb` to download dataset, then `01_eda_cpt.ipynb` to generate preprocessed data
...
Notebooks are at repository root and use simple relative paths:
```python
...
1. Open `00_setup.ipynb` and run all cells to download and extract the dataset
2. Open `01_eda_cpt.ipynb` and run all cells sequentially
...
### Model Training
1. Ensure `data/model_ready/` contains train/test CSVs (run `01_eda_cpt.ipynb` first)
2. Open `03_training_evaluation.ipynb`
